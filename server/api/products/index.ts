...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    if (!search && !categoryId && !brandId) {
      ...
      let result = await getProductsList(targetPage, perPage, search, categoryId, brandId)
      ...
      if (result.items.length === 0 && targetPage > actualTotalPages && actualTotalPages > 0) {
        ...
        result = await getProductsList(newTargetPage, perPage, search, categoryId, brandId)
      }
